...
UIButton::UIButton(std::string overReference, /*std::string title,*/ std::string imageName, std::string hoverImageName, std::string pressedImageName,
	float posX, float posY, float sizeX, float sizeY) : UI_Element()
{
	//panel = static_cast<Ogre::OverlayContainer*>(overlayMan_->createOverlayElement("Panel", title + std::to_string(nOverlay)));
